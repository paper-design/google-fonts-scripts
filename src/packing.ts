...
const createFontBundles = async () => {
  ...
  for (const [fontName, weights] of fontEntries) {
    ...
    try {
      ...
      console.warn(`⚠️  Error processing ${fileName}.png for font "${fontName}": ${error instanceof Error ? error.message : error}`)
      missingFiles++
    }
  }
...
createFontBundles()
  .then(result => {
    ...
    result.chunks.forEach(chunk => {
      console.log(`   Chunk ${chunk.chunkNumber}: ${chunk.totalImages} fonts, ${chunk.width}x${chunk.height}px, ${(chunk.fillRatio * 100).toFixed(1)}% fill`)
    })
  })
  .catch(error => {
